...
function menuItem() {
	...
	$.ajax({
		type:'POST',
		async: false,
		url:ctx+"/sysUser/checkLogin.html",
		success:function(msg){
			flag=msg.flag;
		}
	});
	if(flag==false){//没有登录或者已经session过期
		$(this).attr('href',ctx);
		windows.location.href="/";
//		windows.location.reload (true); //location.replace(URL)//=ctx;
		return;
	}*/
    // 获取标识数据
            ...
            if (!$(this).hasClass('active')) {
                ...
                $('.J_mainContent .J_iframe').each(function () {
                    if ($(this).data('id') == dataUrl) {
                        ...
                        $(this).attr('src', dataUrl);//刷新iframe
                        return false;
                    }
                });
            }else{
				 $('.J_mainContent .J_iframe').each(function () {
	                    if ($(this).data('id') == dataUrl) {
	                        $(this).show().siblings('.J_iframe').hide();
	                        //$('.J_mainContent .J_iframe').attr('src', dataUrl);
							$(this).attr('src', dataUrl);//刷新iframe
	                        return false;
	                    }
	                });
