...
$(function(){
   ...
   // 列表项双击播放
    $(".music-list").on("dblclick",".list-item", function() {
        ...
        var musicname = target.html();
        if(isNaN(num)||musicname.indexOf("蜡笔小鑫正在努力争取版权")!=-1) return false;
        listClick(num);
    });
    ...
    $(".music-list").on("mousemove",".list-item", function() {
        ...
        if(!$(this).data("loadmenu")) {
            ...
            '</div>';
            //alert(target.html());
            if(html.indexOf("蜡笔小鑫正在努力争取版权")!=-1){
            	html = '<span class="music-name-cult">' + 
            	target.html() + 
            	'</span>' ;
            }
            target.html(html);
    ...
    $("#sheet").on("click",".login-in", function() {
       var divHtml =
       '<div id="header">'+
       '	<form class="activate-from">'+
	   '		<input type="text" class="input_key" id="user_id_input" maxlength="100" placeholder="请输入您的用户ID" autocomplete="off"/>'+
	   '	</form>'+
	   '</div>';
       /*
       layer.open({
        ...
        btn3: function(index, layero){
        	layer.open({
		        ...
		        ,content:
		        	'<div style="padding: 35px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 430px;">'+
		        	'1、首先<a style="color:#8DC154" href="http://music.163.com/" target="_blank">点我(http://music.163.com/)</a>打开网易云音乐官网<br>' +
                    '2、然后点击页面右上角的“登录”，登录您的账号<br>' + 
                    '3、点击您的头像，进入个人中心<br>' + 
                    '4、此时<span style="color:#fff">浏览器地址栏</span> <span style="color: #8DC154">/user/home?id=</span> 后面的<span style="color:#FEB581">数字</span>就是您的网易云 UID'+
		        	'</div>'
		      });
        	},
        btnAlign: 'c',
     ...
     layer.msg(divHtml, {
        ...
        btn3: function(index, layero){
        	layer.open({
		        ...
		        ,content:
		        	'<div style="padding: 35px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 430px;">'+
		        	'1、首先<a style="color:#8DC154;text-decoration:underline;"  href="http://music.163.com/" target="_blank">点我(http://music.163.com/)</a>打开网易云音乐官网<br>' +
                    '2、然后点击页面右上角的“登录”，登录您的账号<br>' + 
                    '3、点击您的头像，进入个人中心<br>' + 
                    '4、此时<span style="color:#fff">浏览器地址栏</span> <span style="color:#8DC154">/user/home?id=</span> 后面的<span style="color:#FEB581">数字</span>就是您的网易云 UID'+
		        	'</div>'
        ...
        btn1: function(index){
        	var vals = $("#user_id_input").val();
            // 输入后的回调函数
            if(isNaN(vals)) {
                layer.msg('用户ID 只能由数字组成噢!',{anim: 6});
                return false;
            }
            layer.close(index);     // 关闭输入框
            ajaxUserList(vals);
...
function musicInfo(list, index) {
    ...
    /*layer.open({
   	...
   	layer.msg(tempStr, {
        ...
        closeBtn: false,
        shade: 0.7,
        id: 'LAY_layuipross', //设定一个id，防止重复弹出
        shadeClose: true,
        anim: 0, //0-6的动画形式，-1不开启
        moveType: 1, //拖拽模式，0或者1
        shadeClose: true
...
// 展现搜索弹窗
function searchBox() {
    var tmpHtml = '<div id="search-area"><form onSubmit="return searchSubmit()">' + 
    '    <div class="search-group">' + 
    '        <input type="text" name="wd" id="search-wd" placeholder="搜索歌手、歌名、专辑" autofocus required>&nbsp;' + 
    '        <button class="magic-button" type="submit">搜 索</button>' + 
    '    </div>' + 
    '    <div class="radio-group" id="music-source">' + 
    '       <input type="radio" class="magic-radio" name="source" value="0002" id="r1" checked > <label for="r1">网易云</label>' + 
    '       <input type="radio" class="magic-radio" name="source" value="0001" id="r2"> <label for="r2">QQ</label>' + 
    '       <input type="radio" class="magic-radio" name="source" value="0005" id="r3"> <label for="r3">虾米</label>' + 
    '       <input type="radio" class="magic-radio" name="source" value="0003" id="r4"> <label for="r4">酷狗</label>' + 
    '       <input type="radio" class="magic-radio" name="source" value="0004" id="r5"> <label for="r5">百度</label>' + 
    '   </div>' + 
    '</form></div>';
...
function searchSubmit() {
    ...
    if(!wd) {
        layer.msg('老铁，搜索内容嘞？', {anim:6, offset: 't'});
        $("#search-wd").focus();
        return false;
    }
